...
Class Turbo_Lib_Krumo {
	...
	*/
	Public Static Function backtrace() {
		...
		// disabled ?
		//
		if (!Turbo_Lib_Turbo_Lib_Krumo::_debug()) {
			return false;
			}

		// render it
		//
		return Turbo_Lib_Turbo_Lib_Krumo::dump(debug_backtrace());
		}
...
This is a list of all currently declared classes.
		...
		<?php
		return Turbo_Lib_Turbo_Lib_Krumo::dump(get_declared_classes());
		}
...
This is a list of all currently declared interfaces.
		...
		<?php
		return Turbo_Lib_Turbo_Lib_Krumo::dump(get_declared_interfaces());
		}
...
This is a list of all currently included (or required) files.
		...
		<?php
		return Turbo_Lib_Turbo_Lib_Krumo::dump(get_included_files());
		}
...
This is a list of all currently declared functions.
		...
		<?php
		return Turbo_Lib_Turbo_Lib_Krumo::dump(get_defined_functions());
		}
...
This is a list of all currently declared constants (defines).
		...
		<?php
		return Turbo_Lib_Turbo_Lib_Krumo::dump(get_defined_constants());
		}
...
This is a list of all currently loaded PHP extensions.
		...
		<?php
		return Turbo_Lib_Turbo_Lib_Krumo::dump(get_loaded_extensions());
		}
...
This is a list of all HTTP request headers.
		...
		<?php
		return Turbo_Lib_Turbo_Lib_Krumo::dump(getAllHeaders());
		}
...
This is a list of the configuration settings read from <code><b><?php echo get_cfg_var('cfg_file_path');?></b></code>.
		...
		<?php
		return Turbo_Lib_Turbo_Lib_Krumo::dump(parse_ini_file(get_cfg_var('cfg_file_path'), true));
		}
...
This is a list of all your configuration settings.
		...
		<?php
		return Turbo_Lib_Turbo_Lib_Krumo::dump(ini_get_all());
		}
...
This is a list of the specified directories under your <code><b>include_path</b></code> option.
		...
		<?php
		return Turbo_Lib_Krumo::dump(explode(PATH_SEPARATOR, ini_get('include_path')));
		}
...
This is a list of all the values from the <code><b>$_REQUEST</b></code> array.
		...
		<?php
		return Turbo_Lib_Krumo::dump($_REQUEST);
		}
...
This is a list of all the values from the <code><b>$_GET</b></code> array.
		...
		<?php
		return Turbo_Lib_Krumo::dump($_GET);
		}
...
This is a list of all the values from the <code><b>$_POST</b></code> array.
		...
		<?php
		return Turbo_Lib_Krumo::dump($_POST);
		}
...
This is a list of all the values from the <code><b>$_SERVER</b></code> array.
		...
		<?php
		return Turbo_Lib_Krumo::dump($_SERVER);
		}
...
This is a list of all the values from the <code><b>$_COOKIE</b></code> array.
		...
		<?php
		return Turbo_Lib_Krumo::dump($_COOKIE);
		}
...
This is a list of all the values from the <code><b>$_ENV</b></code> array.
		...
		<?php
		return Turbo_Lib_Krumo::dump($_ENV);
		}
...
This is a list of all the values from the <code><b>$_SESSION</b></code> array.
		...
		<?php
		return Turbo_Lib_Krumo::dump($_SESSION);
		}
...
This is a list of all the values from the <code><b><?php echo realpath($ini_file) ? realpath($ini_file) : $ini_file;?></b></code> INI file.
		...
		<?php
		return Turbo_Lib_Krumo::dump($_);
		}

	// -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- 

	Public Static Function setupCssAndJS(){
		echo "<style>" . file_get_contents(dirname(__FILE__) . "/Krumo/krumo.css") . "</style>";
		echo "<script type=\"text/javascript\">" . file_get_contents(dirname(__FILE__) . "/Krumo/krumo.js") . "</script>";
	}
	/**
	...
	*/
	Public Static Function dump($data) {
		...
		Turbo_Lib_Krumo::setupCssAndJS();
		// more arguments ?
		//
		if (func_num_args() > 1) {
			$_ = func_get_args();
			foreach($_ as $d) {
				Turbo_Lib_Krumo::dump($d);
				}
			return;
			}
...
<div class="krumo-root">
	<ul class="krumo-node krumo-first">
		<?php echo Turbo_Lib_Krumo::_dump($data);?>
		<li class="krumo-footnote">
			<div class="krumo-version" style="white-space:nowrap;">
				<h6>Krumo version <?php echo Turbo_Lib_Krumo::version();?></h6> | <a
...
<?php
		// flee the hive
		//
		$_recursion_marker = Turbo_Lib_Krumo::_marker();
		if ($hive =& Turbo_Lib_Krumo::_hive($dummy)) {
		...
		if (is_array($data) && version_compare(PHP_VERSION, "5", "<")) {
			unset($GLOBALS[Turbo_Lib_Krumo::_marker()]); 
			}
		}
	...
	*/
	Private Static Function _css() {
		...
		$css = '';
		$skin = Turbo_Lib_Krumo::_config('skin', 'selected', 'default');
		...
		if ($_css = $css != '') {
			...
			// fix the urls
			//
			$css_url = Turbo_Lib_Krumo::_config('css', 'url') . "skins/{$skin}/";
...
<?php
	...
	*/
	Public Static Function enable() {
		return true === Turbo_Lib_Krumo::_debug(true);
		}
	...
	*/
	Public Static Function disable() {
		return false === Turbo_Lib_Krumo::_debug(false);
		}
	...
	*/
	Private Static Function _dump(&$data, $name='...') {
		...
		//
		if (is_object($data)) {
			return Turbo_Lib_Krumo::_object($data, $name);
			}
		...
		if (is_array($data)) {
			...
			if (version_compare(PHP_VERSION, "5", "<")) {
				...
				//
				if (!isset($GLOBALS[Turbo_Lib_Krumo::_marker()])) {
					$GLOBALS[Turbo_Lib_Krumo::_marker()] = array();
					}
				if (!is_array($GLOBALS[Turbo_Lib_Krumo::_marker()])) {
					$GLOBALS[Turbo_Lib_Krumo::_marker()] = (array) $GLOBALS[Turbo_Lib_Krumo::_marker()];
					}
				
				// extract ?
				//
				if (!empty($GLOBALS[Turbo_Lib_Krumo::_marker()])) {
					$d = array_shift($GLOBALS[Turbo_Lib_Krumo::_marker()]);
					if (is_array($d)) {
						$data = $d;
						}
					}
			...
			return Turbo_Lib_Krumo::_array($data, $name);
			}

		// resource ?
		//
		if (is_resource($data)) {
			return Turbo_Lib_Krumo::_resource($data, $name);
			}
		
		// scalar ?
		//
		if (is_string($data)) {
			return Turbo_Lib_Krumo::_string($data, $name);
			}
		
		if (is_float($data)) {
			return Turbo_Lib_Krumo::_float($data, $name);
			}

		if (is_integer($data)) {
			return Turbo_Lib_Krumo::_integer($data, $name);
			}

		if (is_bool($data)) {
			return Turbo_Lib_Krumo::_boolean($data, $name);
			}
		
		// null ?
		//
		if (is_null($data)) {
			return Turbo_Lib_Krumo::_null($name);
			}
		}
...
<?php
	...
	*/
	Private Static Function _vars(&$data) {
		...
		//
		if ($_r > 0) {
			return Turbo_Lib_Krumo::_recursion();
			}

		// stain it
		//
		Turbo_Lib_Krumo::_hive($data);
...
/**
* Alias of {@link Turbo_Lib_Krumo::dump()}
*
* @param mixed $data,...
*
* @see Turbo_Lib_Krumo::dump()
*/
Function krumo() {
	$_ = func_get_args();
	return call_user_func_array(
		array('Turbo_Lib_Krumo', 'dump'), $_
		);
	}
